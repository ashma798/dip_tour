...
function _MakePWDWidget(){
	var code=""; var pwdname = this.pwdobjname;
	this.forminputbigid = pwdname+"_id";
	code += "<label class='control-label'>Password</label><input type='password' onpaste='return false;' autocomplete='off' minlength='5' maxlength='25' placeholder='Password' class='nornput padpas border5 form-control' name='"+pwdname+"' id='"+this.forminputbigid+"'>";

	this.pwdtxtfield=pwdname+"_text";
	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	code += "<input type='text' onpaste='return false;' autocomplete='off' minlength='5' maxlength='25' placeholder='Password' class='nornput padpas border5 form-control' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' style='display: none;'>";
...
function showGeneatedPwd()
{
	...
	document.getElementById("confirm_login_password").value = pwd;
	$('#login_password').html('<span class="success">Valid Password</span>');
	$("#confirm_login_password").removeClass( "idev errordiv" ).addClass("idev successdiv" );
	this._showPasswordStrength();
}


function generatePWD(){
    var maxAlpha = 26; var strSymbols="~!@#$%^&*(){}?><`=-|]["; var password='';
